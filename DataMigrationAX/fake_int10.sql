...
with prh_max as
(Select [Order No_], cast(max([Posting Date]) as Date) as PostDate
from urban_NAV600.dbo.[Urban-Brand GmbH$Purch_ Rcpt_ Header] as prh with (NOLOCK)
group by [Order No_]
)

Select distinct ph.No_, prh_max.PostDate, cast(ph.[Order Date] as Date) as OrderDate,ph.[Buy-from Vendor No_],@venshipno as ShipNo,pl.[Line No_],pl.No_, 
...
Where pl.[Posting Group] = 'HANDEL' and ([Quantity Received]-[Quantity Invoiced]) <> '0' --and ph.No_= 'BEST-1099536'
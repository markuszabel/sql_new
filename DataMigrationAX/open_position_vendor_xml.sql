...
(
Select 
[Vendor Ledger Entry No_] as EntryNo, sum(Amount) as Wert, [Initial Entry Global Dim_ 1] as KostStell, [Initial Entry Global Dim_ 2] as KostTraeg,
case [Currency Code]
when '' then 'EUR'
else [Currency Code]
end as CurrencyCode
from urban_NAV600.dbo.[Urban-Brand GmbH$Detailed Vendor Ledg_ Entry] with (NOLOCK)
Group by [Vendor Ledger Entry No_],[Initial Entry Global Dim_ 1], [Initial Entry Global Dim_ 2],[Currency Code]
)
...
cle.[Vendor No_] "LedgerDimension", 
...
from urban_NAV600.dbo.[Urban-Brand GmbH$Vendor Ledger Entry] as cle with (NOLOCK)
Left Join dcu

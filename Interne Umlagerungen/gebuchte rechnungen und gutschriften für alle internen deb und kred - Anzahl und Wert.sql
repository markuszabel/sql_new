/**  Auswertung 1462 - Intern - Gebuchte Rechnungen und Gutschriften mit Anzahl und Wert **/

Select pih.No_, pih.[Buy-from Vendor No_],cast(sum(pil.Quantity)as INT) as Menge, cast(sum(pil.Amount) as float) as Wert
...
Group by pih.No_,pih.[Buy-from Vendor No_]

Select pch.No_, pch.[Buy-from Vendor No_],cast(sum(pcl.Quantity*-1)as INT) as Menge, cast(sum(pcl.Amount*-1) as float) as Wert
...
Group by pch.No_,pch.[Buy-from Vendor No_]

Select sih.No_, sih.[Sell-to Customer No_],cast(sum(sil.Quantity*-1)as INT) as Menge, cast(sum(sil.Amount*-1) as float) as Wert
...
Group by sih.No_,sih.[Sell-to Customer No_]

Select sch.No_, sch.[Sell-to Customer No_],cast(sum(scl.Quantity)as INT) as Menge, cast(sum(scl.Amount) as float) as Wert
...
Group by sch.No_,sch.[Sell-to Customer No_]

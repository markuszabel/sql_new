...
from
(/*** EKRs ****/
select max(cast(ve.[Posting Date] as date)) as BuchungsdatumEKR,
ve.[Item Ledger Entry No_],
ve.[Document No_] as EKR,
sum(ve.[Cost Amount (Expected)]) as CostAmountExpected
from urban_NAV600.dbo.[Urban-Brand GmbH$Value Entry] as ve
where ve.[Source No_] = '71066' and ve.[Location Code]='BER_FIEGE' and ve.[Posting Date] <= '2015-03-31' and ve.[Document Type] = 6
...
(/***** EKLs ******/
select
cast(ve.[Posting Date] as date) as BuchungsdatumEKL,
ve.[Item Ledger Entry No_],
ve.[Document No_] as EKL,
sum(ve.[Cost Amount (Expected)]) as CostAmountExpected
from urban_NAV600.dbo.[Urban-Brand GmbH$Value Entry] as ve
where ve.[Document Type] = 5 and ve.[Source No_] = '71066' and ve.[Location Code]='BER_FIEGE' and ve.[Posting Date] > '2015-03-31'
Group by
ve.[Posting Date],
ve.[Item Ledger Entry No_],
ve.[Document No_]
